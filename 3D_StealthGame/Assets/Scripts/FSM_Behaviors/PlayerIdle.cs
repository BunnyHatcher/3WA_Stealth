...
public class PlayerIdle : CharacterStateBase
{
    ...
    override public void OnStateUpdate(Animator animator, AnimatorStateInfo stateInfo, int layerIndex)
    {
       ...
       + GetPlayerControls(animator)._cameraTransform.right * Input.GetAxisRaw("Horizontal");
        ...
        GetPlayerControls(animator)._controller.Move(GetPlayerControls(animator)._movementDirection.normalized * GetPlayerControls(animator)._currentSpeed * Time.deltaTime);
        ...
        GetPlayerControls(animator).transform.rotation = Quaternion.Lerp(GetPlayerControls(animator).transform.rotation, lookRotation, GetPlayerControls(animator)._turnSpeed * Time.fixedDeltaTime);
    }
